    ...
    <>
      <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-3 sm:px-6 md:px-8 bg-zinc-950/70 backdrop-blur shrink-0 z-30 transition-all relative">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          {/* Sign In with Google / Cloud Sync Indicator */}
          {!currentUser ? (
            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              onClick={signInWithGoogle}
              className="flex items-center gap-2 px-3 py-1.5 bg-white hover:bg-zinc-100 text-zinc-950 font-bold rounded-xl text-xs shadow-sm transition-all border border-zinc-200 cursor-pointer shrink-0"
              title="Sign in with your Google account to sync across all your devices"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold transition-all border shadow-xs cursor-pointer bg-zinc-900/90 hover:bg-zinc-800 border-zinc-800 hover:border-emerald-500/50 text-zinc-300"
              title={`Google Account: ${currentUser.email} (${syncStatus === 'syncing' ? 'Syncing...' : 'Cloud Synced'})`}
            >
              <Cloud
                className={`w-3.5 h-3.5 ${
                  syncStatus === 'syncing' ? 'text-amber-400 animate-pulse' : 'text-emerald-400'
                }`}
              />
